...
# For each file in the region folder,
regions = os.listdir("./region")
for region_counter, file in enumerate(regions):
	if file.endswith(".mca"):
		...
		print(f"Region {region_counter + 1}/{len(regions)}: {len(structures)} structures found")
		...
		print(f"Region {region_counter + 1}/{len(regions)}: {len(structures)} structures saved")
